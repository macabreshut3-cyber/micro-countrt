...
import { AppState, MediumType, ColonyData } from './types';
...
export default function App() {
  ...
  const handleMediumSelect = (medium: MediumType) => {
    setState((s) => ({ ...s, medium, step: 'camera' }));
  };
      ...
      <main className="flex flex-1 overflow-hidden relative">
        <div className="flex-1 overflow-y-auto w-full h-full">
          <div className="min-h-full p-4 md:p-8">
            {state.step === 'start' && (
              <StartScreen onSelectMedium={handleMediumSelect} />
            )}
            ...
            {state.step === 'detection' && state.calibratedImageSrc && state.medium && (
              <ColonyDetection
            ...
            {state.step === 'review' && state.calibratedImageSrc && state.medium && (
              <ManualReview
                imageSrc={state.calibratedImageSrc}
                medium={state.medium}
            ...
            {state.step === 'result' && state.calibratedImageSrc && state.medium && (
              <ResultSummary
                medium={state.medium}
